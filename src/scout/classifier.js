...
const SUCCESS_RE = /thank you|thanks for submitting|submitted|pending review|we received|审核|提交成功|已提交/i;
...
function mappedRequiredFields(forms = []) {
  return requiredFields(forms).filter(field => PRODUCT_FIELD_KEYS.has(field.mapped_to));
}
