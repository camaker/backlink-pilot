...
Automated backlink submission for indie products. Product config is generated by command, not hand-edited.
...
Use `--engine bb`. The CLI auto-starts `bb-browser` daemon when possible.

## Commands

```bash
node src/cli.js init --url <product-url>        # generate local config
...
node src/cli.js submit <site> --product-url <url> --engine bb
...
1. Do not require manual `config.yaml` edits; run `init --url` or pass `--product-url`
