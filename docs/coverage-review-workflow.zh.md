...
```bash
node src/cli.js targets coverage-review-draft \
  backlink-url/review-batches/p0-001.csv \
  backlink-url/review-batches/p0-001-suggestions.csv \
  --output backlink-url/review-batches/p0-001.draft.csv \
  --json-output backlink-url/review-batches/p0-001-draft-report.json \
  --min-confidence high \
  --reviewed-by read_only_evidence
```

Draft 规则：

- 只写入高置信拒绝建议。
- 默认只允许 `reject_auth_required`、`reject_duplicate`、`reject_not_submit`、`reject_paid`。
- 不生成 `approved` 或 `approved_domain_variant`。
- 已经有 `review_decision` 的行不会被覆盖。
- Draft 仍然只是批次 CSV，必须经过 `validate-coverage-review-batch` 和 `promote-coverage-review-batch --dry-run`。
